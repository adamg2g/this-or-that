...
A 4-player party game in a single HTML file. Players open the same URL, enter their names, and once 4 have joined the game runs 4 rounds of "this or that" questions. After each round, a reveal screen shows a token with each player's initials under the option they chose.
...
Edit the `QUESTIONS` array at the top of the `<script>` block in `index.html` — four `{ a, b }` pairs, where `a` is the coral card and `b` is the teal card.
